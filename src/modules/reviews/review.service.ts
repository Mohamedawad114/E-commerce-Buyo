...
import { type IUser, productRepo, ReviewRepo, Sys_Role } from 'src/common';
...
export class Review_Services {
  ...
  addReview = async (
    ...
    if (create) {
      const newAvgRating =
        (product.rateAvg * product.reviewsNumber + Dto.stars) /
  ...
  getProductReviews = async (productId: Types.ObjectId, page: number) => {
    const product = await this.productRepo.findByIdDocument(
      productId
    );
    if (!product) throw new NotFoundException('product not found');
    const limit = 15;
    ...
    const reviews = await this.reviewRepo.findDocuments(
      { productId: productId },
      {},
      { limit, skip: offset, populate: { path: 'userId', select: 'username profilePicture' } },
    );
